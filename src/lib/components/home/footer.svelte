	...
	<div class="flex flex-col items-center gap-4 sm:flex-row sm:justify-between">
		...
		<ul class="text-center">
			{#each navItems as item}
				...
				>
					<a
						class="inline-flex gap-x-2 text-sm text-gray-500 hover:text-gray-800 focus:text-gray-800 focus:outline-hidden dark:text-neutral-500 dark:hover:text-neutral-200 dark:focus:text-neutral-200"
						href={item.href}
					>
						{item.title}
					</a>
